...
interface RunnerCardProps {
  ...
  onCreatePod?: (runnerId: number) => void;
}
...
export function RunnerCard({ runner, onDelete, onCreatePod }: RunnerCardProps) {
  const statusStyle = statusConfig[runner.status] || statusConfig.offline;
  const canCreatePod =
    runner.status === "online" &&
    runner.currentPods < runner.maxConcurrentPods;
        ...
        <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${
              runner.currentPods >= runner.maxConcurrentPods
                ? "bg-red-500"
                : runner.currentPods >= runner.maxConcurrentPods * 0.8
                ? "bg-yellow-500"
                : "bg-green-500"
            }`}
            style={{
              width: `${(runner.currentPods / runner.maxConcurrentPods) * 100}%`,
            }}
          />
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {canCreatePod && (
          <Button
            ...
            variant="default"
            className="flex-1"
            onClick={() => onCreatePod?.(runner.id)}
          >
            New Pod
          </Button>
        )}
        {!canCreatePod && runner.status === "online" && (
