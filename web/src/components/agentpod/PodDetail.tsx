...
export function PodDetail({ podKey }: PodDetailProps) {
  ...
  const { terminalRef, connect, disconnect, sendInput, sendResize } = useTerminal(podKey);
  ...
  // Auto-connect terminal when pod is running
  useEffect(() => {
    if (pod?.status === "running" && !isConnected) {
      handleConnect();
    }
  }, [pod?.status]);
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <Button onClick={fetchPod}>Retry</Button>
      </div>
    );
  }
        ...
        <div className="w-80 flex-shrink-0 border-l border-border p-4 overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="border border-border rounded-lg p-4">
              <h3 className="font-medium mb-3">Pod Info</h3>
              <dl className="space-y-2 text-sm">
                {pod.agent_type && (
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Agent</dt>
                    <dd className="font-medium">{pod.agent_type.name}</dd>
                  </div>
                )}
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Agent Status</dt>
                  <dd className="font-medium">{pod.agent_status || "—"}</dd>
                </div>
                {pod.runner && (
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Runner</dt>
                    <dd className="font-mono text-xs">{pod.runner.node_id}</dd>
                  </div>
                )}
                {isActive && (
                  <div className="flex justify-between">
              ...
              <div className="border border-border rounded-lg p-4">
                ...
                <dl className="space-y-2 text-sm">
                  <div>
                    ...
                    <dd className="font-medium truncate" title={pod.repository.full_path}>
                      {pod.repository.full_path}
                    </dd>
                  </div>
                  {pod.branch_name && (
                    <div>
                      ...
                      <dd className="flex items-center gap-1">
                        ...
                        <span className="font-mono text-xs">{pod.branch_name}</span>
                      </dd>
                    </div>
                  )}
                </dl>
            ...
            {pod.ticket && (
              <div className="border border-border rounded-lg p-4">
                ...
                <div
                  ...
                  onClick={() => router.push(`/tickets/${pod.ticket!.identifier}`)}
                >
                  <div className="text-xs text-primary font-mono mb-1">
                    {pod.ticket.identifier}
                  </div>
                  <div className="text-sm truncate">{pod.ticket.title}</div>
                </div>
              </div>
            )}
            ...
            <div className="border border-border rounded-lg p-4">
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd>{formatTime(pod.created_at)}</dd>
                </div>
                {pod.started_at && (
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Started</dt>
                    <dd>{formatTime(pod.started_at)}</dd>
                  </div>
                )}
                {pod.finished_at && (
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Finished</dt>
                    <dd>{formatTime(pod.finished_at)}</dd>
                  </div>
                )}
                {pod.last_activity && (
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Last Activity</dt>
                    <dd>{formatTime(pod.last_activity)}</dd>
                  </div>
                )}
              </dl>
            </div>
              ...
              <div className="border border-border rounded-lg p-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-sm font-medium">
                    {(pod.created_by.name || pod.created_by.username)[0].toUpperCase()}
                  </div>
                  <div>
                    <div className="text-sm font-medium">
                      {pod.created_by.name || pod.created_by.username}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      @{pod.created_by.username}
                    </div>
                  </div>
                </div>
              </div>
...
function PodDetailSkeleton() {
  return (
    <div className="animate-pulse" data-testid="pod-detail-skeleton">
