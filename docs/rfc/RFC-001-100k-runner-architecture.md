...
AgentMesh 平台需要支撑大规模用户自托管的 Runner 连接。Runner 是用户部署在自己机器上的运行节点，承载多个 AgentPod（AI Agent 运行实例）。随着用户规模增长，平台需要能够处理 10 万级别的并发 WebSocket 长连接。
...
| SessionCoordinator | `backend/internal/service/runner/session_coordinator.go` | 协调 AgentPod 生命周期 |
...
func (h *Hub) Run() {
    for {
        select {
        case msg := <-h.sessionBroadcast:  // 单 goroutine 串行处理
            // ...
        }
    }
}
...
| Scrollback | `scrollback:{session_id}` | 3600s | 终端回滚缓冲（LRU evict 时持久化） |
...
AgentMesh 的资源模型以 Organization 为边界：

```
Organization
    ├── Runners (属于 Org)
    ├── AgentPods/Sessions (属于 Org)
    ├── Tickets (属于 Org)
...
│                                                                             │
│   结果：                                                                     │
│   ✅ 同一 Org 的 Runner、Session、前端请求 → 同一 Backend                    │
...
```
wss://api.agentmesh.io/ws/runner?node_id=xxx&token=xxx
...
```
wss://api.agentmesh.io/ws/terminal?org_id=123&session_id=xxx
```
...
func (tr *TerminalRouter) getScrollback(sessionID string) *ScrollbackBuffer {
    // 1. Check LRU cache
    if buf, ok := tr.scrollbackCache.Get(sessionID); ok {
        return buf
    }

    // 2. Load from Redis
    ctx := context.Background()
    data, err := tr.redis.Get(ctx, "scrollback:"+sessionID).Bytes()
    ...
    tr.scrollbackCache.Add(sessionID, buf)
    return buf
}

func (tr *TerminalRouter) saveScrollback(sessionID string, buf *ScrollbackBuffer) {
    ctx := context.Background()
    data := buf.GetData()
    if len(data) > 0 {
        tr.redis.Set(ctx, "scrollback:"+sessionID, data, time.Hour)
    }
}
...
func (tr *TerminalRouter) handleTerminalOutput(runnerID int64, data *TerminalOutputData) {
    ...
    buf := tr.getScrollback(sessionID)
    buf.Write(data.Data)

    // 异步保存到 Redis（每 N 秒或每 N 次写入）
    go tr.saveScrollback(sessionID, buf)
...
func (sh *ShardedHub) getShard(sessionID string) *Hub {
    h := fnv.New32a()
    h.Write([]byte(sessionID))
    return sh.shards[h.Sum32()%hubShards]
}

func (sh *ShardedHub) BroadcastToSession(sessionID string, message []byte) {
    hub := sh.getShard(sessionID)
    hub.BroadcastToSession(sessionID, message)
}

func (sh *ShardedHub) RegisterClient(client *Client) {
    hub := sh.getShard(client.SessionID)
    hub.Register(client)
}
...
-- AgentPod 查询优化
CREATE INDEX idx_sessions_runner_status ON sessions(runner_id, status);

-- 终端活动查询
CREATE INDEX idx_sessions_activity ON sessions(last_activity_at) WHERE status = 'active';
```
...
| 指标 | 告警阈值 | 说明 |
|------|----------|------|
| `agentmesh_runner_connections_total` | > 12,000/实例 | 连接数过高 |
| `agentmesh_heartbeat_latency_p99` | > 500ms | 心跳延迟 |
| `agentmesh_db_connections_used_ratio` | > 80% | 连接池饱和 |
| `agentmesh_redis_memory_used_ratio` | > 80% | Redis 内存 |
| `agentmesh_websocket_error_rate` | > 0.1% | WebSocket 错误 |
...
3. **终端数据测试**：10% 活跃 session，5KB/s 数据流
4. **故障恢复测试**：单实例宕机后的重连
